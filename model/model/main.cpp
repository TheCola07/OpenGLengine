...
Imported myModel("E:/openGL/计算机图形学/计算机图形学/Curriculumdesign1/bunny.obj");
Imported myModel2("E:/openGL/计算机图形学/计算机图形学/Curriculumdesign1/shuttle.obj");
...
int main()
{
    ...
    unsigned char* data = stbi_load("/Users/bloodsvery/Desktop/学习/大二下/计算机图形学/Curriculumdesign1/sun.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // The FileSystem::getPath(...) is part of the GitHub repository so we can find files on any IDE/platform; replace it with your own image path.
    unsigned char* data2 = stbi_load("/Users/bloodsvery/Desktop/学习/大二下/计算机图形学/Curriculumdesign1/earth.jpg", &width, &height, &nrChannels, 0);
    if (data2)
    {
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    if (key == GLFW_KEY_Z) size1 += 0.03;//模型放大

    if (key == GLFW_KEY_X) size1 -= 0.03;//模型缩小
