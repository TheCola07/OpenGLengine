	...
	while (!f.eof()) {//返回文件结束符号
		...
		for (int i = 0; i < line.length(); i++) {
			...
			if (ch != ' ') {
				...
				if (ans == "")
					continue;
				parameters.push_back(ans);//取出字符串中的元素，空格作为分割符
				ans = "";
			}
		}
		...
		if (parameters.size() <= 2 || parameters.size() >= 5) {
			...
			if (parameters[0] == "v") {//如果是顶点的话
				...
				for (int i = 1; i < 4; i++) {	//从1开始，将顶点的xyz三个坐标放入存放顶点的vector中(第一位为标识符)
					GLfloat componet = atof(parameters[i].c_str());
					Point.push_back(componet);
				}
				vSets.push_back(Point);
			} else if (parameters[0] == "f") {	//如果是面的话，则存放以三角型为基础面的三个顶点的索引值
